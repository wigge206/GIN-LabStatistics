...
### Novel therapies of treatments to improve blood haemoglobin
This is a hypothetical dataset made to compare several treatments to standards care. The change in blood haemoglobin from baseline was recorded for 10 individuals on each of the six novel therapies (named DrugA-DrugF) and the existing standard treatment.
...
sheetNames<-getSheetNames("DummyData_MTT.xlsx") ## Gives you a list of sheetNames in a given excel workbook

## Read a single sheet (the first sheet) from a workbook 
# for details on function type ?function (e.g. ?read.xlsx)
Exp1 <- read.xlsx("DummyData_MTT.xlsx", sheet=sheetNames[1]) # [1] denotes which sheet to read in

## Read all the sheets
KD_data<-lapply(sheetNames,function(i) read.xlsx("DummyData_MTT.xlsx", sheet=i))
...
```{r KDdata, warning=F}
dat <- KD_data %>% `[[`('Exp2') %>% filter(cellLine == "AN3CA")
dat<-dat[1:6]
...
tScore %>% kbl(format = 'html', caption = "t-table for some commonly used df", table.attr =  'style= " width: 30%;"', col.names = c('df',"95%", "99%")) %>% kable_classic(html_font = "Cambria", font_size=16) %>% footnote(general = "df=n-1", general_title = "", escape = F)
```
### Reference
...
## classic t.test - Scramble treated vs siRNA1 treated cells
### as an example the t.test using the formula is written but commented out - the values.both data needs to be generated to test this
...
```{r t.testMultiple}
## Focus on one cell line for now. Drop reagents and cellonly (used to normalise)
...
Although a one-way ANOVA may be considered, there is an issue with the independence of each siRNA as the are targeting the same gene.


# ANOVA

In the case were there are multiple experimental groups, pairwise t-test are inappopriate as the error rate quickly increases. In these cases multiple comparisons need to be considered. Analysis of variance (ANOVA), is the term for a collection of methods to estimate significance between group means. The error rate is reduced because the error is calculated for the whole set of comparisons, not pairwise as would be for a t-test.
...
## One-way ANOVA
To look at how to do this in R, lets use the in build dataset `chickwts` which has the weight of chicks on different feeds.
...
In `R` there are many ways to do ANOVA and tukey post-hoc (also called "Tukey Honest Significant Differences"). For the purpose of labeling plots I prefer the `tukey_hsd()` function from `rstatix`. However, the `tukeyHSD()` funciton in the base `stats` package will give identical results, just in a different structure.
...
```{r Tukeys,  fig.align='center', fig.height=3, echo=F, warning=F, fig.cap="Tukeys multiple comparisons. A) Difference between pairwise comparison with 95% CI. B) The effect of feed on chick weight. Statistical analysis by one-way ANOVA with Tukey's post-hoc test. * p < 0.5, ** p < 0.001, p < 0.0001 "}
stat.test<-aov(weight ~ feed,data = chickwts) %>% tukey_hsd()
...
p0 <-ggplot(stat.test3, aes(x=comparison, y=estimate)) + geom_point() +coord_flip() +theme_bw(base_size = 8) + geom_errorbar(aes(ymin=conf.low, ymax=conf.high)) +theme(axis.title.y = element_blank()) + geom_hline(yintercept = 0, linetype=2, colour ='red')+
...
```{r dunnetBxplot, echo=F,  fig.width=4.5, fig.height=2.5, fig.cap="Effect of treatment on blood haemoglobin in amenic individuals. Statistical analysis by one-way ANOVA with Dunnett post-hoc test. * p < 0.5, ** p < 0.01, p < 0.001"}
...
dunnetts %>% bxplot(aes(x=fct_inorder(Technique), y=Score)) + 
  ...
  ylab("Increase in blood haemoglobin (%)") +xlab(NULL)
```
